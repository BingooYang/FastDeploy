...
int get_scales_cache_size(thread_config_t const& th_config,
  ...
  if (cache_scales_chunk) {
    ...
    load_groups =
        std::max(load_groups, 32);  // We load at least 32 scale groups
...
int get_kernel_cache_size(thread_config_t const& th_config,
  ...
  int sh_s_size = get_scales_cache_size(th_config,
                                        prob_m,
                                        prob_n,
                                        prob_k,
                                        num_bits,
...
bool is_valid_config(thread_config_t const& th_config,
  ...
  int cache_size = get_kernel_cache_size(th_config,
                                         m_block_size_8,
                                         thread_m_blocks,
                                         prob_m,
                                         prob_n,
...
exec_config_t determine_exec_config(
    const MARLIN_NAMESPACE_NAME::ScalarType& q_type,
    int prob_m,
    int prob_n,
    int prob_k,
    int thread_m_blocks,
...
void marlin_mm(const void* A,
               ...
               int prob_k,
               void* workspace,
               MARLIN_NAMESPACE_NAME::ScalarType const& q_type,
               bool has_act_order,
               bool is_k_full,
               bool has_zp,
               int num_groups,
  ...
  PADDLE_ENFORCE(is_valid_config(thread_tfg,
                                 m_block_size_8,
                                 thread_m_blocks,
                                 prob_m,
                                 prob_n,
...
// Explicit template instantiations for Marlin kernels.
// These must be in the same TU as the caller to avoid nvcc cross-TU
// __device_stub__ symbol mismatch.
// clang-format off
namespace MARLIN_NAMESPACE_NAME {
  ...
  // kernel_bf16_ku4b8
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 256, 1, 8, 8, true, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 128, 1, 8, 4, true, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 256, 1, 8, 8, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 128, 1, 8, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 256, 2, 16, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 128, 2, 8, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 256, 3, 16, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 128, 3, 8, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 256, 4, 16, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
  template __global__ void Marlin<nv_bfloat16, MARLIN_NAMESPACE_NAME::kU4B8.id(), 128, 4, 8, 4, false, pipe_stages, 0, false>( MARLIN_KERNEL_PARAMS );
...
}  // namespace MARLIN_NAMESPACE_NAME (explicit instantiations)
// clang-format on
...
MARLIN_NAMESPACE_NAME::Tensor moe_wna16_marlin_gemm(
    ...
    bool use_fp32_reduce,
    bool is_zp_float) {
  MARLIN_NAMESPACE_NAME::ScalarType const b_q_type =
      MARLIN_NAMESPACE_NAME::ScalarType::from_id(b_q_type_id);
  ...
  if (a.dtype() == paddle::DataType::FLOAT16) {
    ...
    MARLIN_NAMESPACE_NAME::marlin_mm<half>(a.data_ptr<DataType>(),
                                           b_q_weight.data_ptr(),
                                           c.data_ptr<DataType>(),
                                           c_tmp.data_ptr<float>(),
                                           scales_ptr,
                                           global_scale.data_ptr<DataType>(),
                                           b_zeros.data_ptr(),
                                           g_idx.data_ptr(),
                                           perm.data_ptr(),
                                           a_tmp.data_ptr<DataType>(),
